    ...
    def __init__(self, env_path: Optional[str] = None, run_options: Dict = {}, use_animation=True, camera_resolution_width=448, camera_resolution_height=448, camera_field_of_view=120, rendering_options: Dict = {}, action_mode=0):
        ...
        if env_path is not None:
            try:
                ...
                # Add headless rendering arguments for Linux servers
                if hasattr(os, 'uname') and os.uname().sysname == "Linux" and os.environ.get("DISPLAY") is None:
                    # Use EGL for headless rendering if available (requires GPU drivers supporting EGL)
                    # -batchmode: Run without window
                    # -nographics: Don't initialize graphics device (CAUTION: This might disable camera rendering in some Unity setups!)
                    # Usually for pixel streaming or server-side rendering, we want GPU but no window.
                    # If we use -nographics, Unity won't render anything to the camera.
                    # So we usually use -batchmode + -nographics ONLY if we don't need camera images.
                    # BUT, if we want camera images on headless server, we typically need Xvfb or EGL.
                    # If the user specifically asked for -use-egl-device-context, we should add it.
                    
                    # NOTE: -nographics will prevent the camera from rendering images! 
                    # If your agent relies on visual input, DO NOT use -nographics.
                    # Instead, just use -batchmode (and maybe -use-egl-device-context if supported).
                    
                    # However, since the user asked specifically about these flags, let's add them conditionally or just -batchmode.
                    # Safe bet for headless rendering with camera:
                    run_args.extend(["-batchmode"])
                    # run_args.extend(["-nographics"]) # DANGEROUS: Will break visual agents
                    run_args.extend(["-use-egl-device-context"]) # Enabled for CUDA/EGL support

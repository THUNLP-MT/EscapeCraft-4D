...
def get_default_env_path(root_folder=CLIENT_FOLDER):
    if get_platform() == "linux" or get_platform() == "linux2":
        candidates = glob.glob(os.path.join(root_folder, "*.x86_64")) + glob.glob(os.path.join(root_folder, "*.x86"))
        if len(candidates) > 0:
            return sorted(candidates, reverse=True)[0]
    elif get_platform() == "darwin":
        candidates = glob.glob(os.path.join(root_folder, "*.app"))
        if len(candidates) > 0:
            return sorted(candidates, reverse=True)[0]
    elif get_platform() == "win32":
        candidates = glob.glob(os.path.join(root_folder, "*.exe"))
        candidates = [c for c in candidates if "UnityCrashHandler" not in c]
        if len(candidates) > 0:
            return sorted(candidates, reverse=True)[0]

...
def generate_scene(
    object_counts: Dict[str, int] = {},#精确指明场景中某些物体的总数量
    receptacle_object_counts={},#指定某个容器比如桌子、沙发上必须放置哪些物体
    room_num=0,#指定要生成的房间数量
    method="proc",#指定生成场景的方法，是procedural（程序化）的缩写
):
    if method == "proc":#先检查method是不是"proc",
        ...
        # global prefabs, interactable_names, kinematic_names, interactable_names_set, kinematic_names_set
        room_types = ["Bedroom", "LivingRoom", "Kitchen", "Bathroom"]#包含了所有可能的房间类型
        room_size_range = {#一个字典，定义了房间的大小范围
        ...
        ###下面确定房间数量和类型
        if room_num == 0:
            room_num = random.randint(1, 4)#如果room数目是0的话，第一行代码会随机生成1-4之间的整数作为这次要生成的房间总数
        if 1 <= room_num <= 4:
            sample_rooms = random.sample(room_types, room_num)#从room_types中根据room_num来抽取房间的类型
            #构建房屋布局蓝图的核心部分
            sample_sizes = [random.randint(*room_size_range[room]) for room in sample_rooms]#得到每个room的大小
            total_size = sum(sample_sizes)#计算房间尺寸的综合。总尺寸代表了整个房屋的相对面积
            sample_ratios = [size / total_size for size in sample_sizes]#将每个房间的绝对尺寸转换为相对于总尺寸的比例（或百分比）
            sampler = RoomSpecSampler(#将计算的信息（房间类型、它们的比例）打包成一个标准化的结构化的对象，这个就是房屋布局的草图
                [
                    RoomSpec(

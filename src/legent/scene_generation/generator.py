...
#得到了roomspec（蓝图）objectdb（仓库材料）housegenerator时总的把房子从无到有造出来
class HouseGenerator:
    ...
    #生成房屋的墙和房间id和dims维度尺寸等等
    def generate_structure(self, room_spec):
        house_structure = generate_house_structure(#调用house.py中的generate_house_structure函数
    ...
    def add_floors_and_walls(#根据2D的图之生成3D结构，会被游戏引擎legent来创建3d模型
        self,
        house_structure,
        room_spec,
        odb,
        ...
        room_num = len(room_spec.room_type_map)#获取房间数量和ID
        ...
        DOOR_PREFAB = odb.MY_OBJECTS["door"][0]#从资源库中获取们、墙、地板的尺寸信息
        ...
        #获取房屋布局图（floorplan）,这是一个2D的数组
        floors = house_structure.floorplan
        # convert 1 in floors to 0，将代表走廊的1换成0了
        ...
        #创建一个空列表，用来存放所有要生成的物体
        floor_instances = []
        # generate walls based on the 0-1 boundaries
        #只要蓝图的格子里面有房间，在对应的3d坐标上放置一个地板和天花板
    ...
    def generate(
        self,
        object_counts: Dict[str, int] = {},#指定某些物体的精确数量
        receptacle_object_counts: Dict[str, Dict[str, int]] = {},#指定容器上放置的物体，比如桌子上放橙子
        room_num=None,
    ):
        odb = self.odb#物体数据库，包含所有可以用的prefab
        prefabs = odb.PREFABS#所有预制件的尺寸和属性信息,从object.py来的
        room_spec = self.room_spec#这个就是房间的规格了

        log("starting...")
        log(room_spec)

        house_structure = self.generate_structure(room_spec=room_spec)#生成房屋结构
        interior_boundary = house_structure.interior_boundary#计算房屋的实际物理尺寸
        ...
        self.placer = RectPlacer((min_x, min_z, max_x, max_z))#二维空间不重合
        ...
        # --- 逻辑修改开始 ---
        # 解析从 SceneGeneration.py 传来的打包字典
        # object_counts 现在实际上是 categorized_counts
        
        #small_object_instances = []
        # --- 逻辑修改开始 ---
        # 解析从 SceneGeneration.py 传来的打包字典
        # object_counts 现在实际上是 categorized_counts
        feature_counts = object_counts.get("feature", {})
        critical_counts = object_counts.get("critical", {})
        # ... 可以继续解析其他分类 ...
        ...
        max_object_types_per_room,
        (min_x, min_z, max_x, max_z),
        # object_counts=object_counts, # <- 不再直接传递整个包

        # 将解析出的分类字典作为独立的参数传递
        feature_object_counts=feature_counts,
        critical_object_counts=critical_counts,

        # 保留 object_counts 用于向后兼容或处理未分类物品
        object_counts=object_counts.get("other", {}),

...
def generate_house_structure(room_spec: RoomSpec, dims: Optional[Tuple[int, int]], unit_size):
    room_ids = set(room_spec.room_type_map.keys())#获取所有的房间id

    generate_dims = None
    if dims != None:
        generate_dims = dims
    elif room_spec.dims is not None:
        generate_dims = room_spec.dims()#选定房间尺寸
    #生成内部边界，创建房屋的外轮廓和形状
    ...
    floorplan = generate_floorplan(#将内部边界划分地板平面图
    ...
    rowcol_walls = find_walls(floorplan=floorplan)#检测墙壁，扫描地板平面图找出相邻房间之间的边界线
    boundary_groups = consolidate_walls(walls=rowcol_walls)#将零散的墙壁线段合并成连续的墙面
    boundary_groups = scale_boundary_groups(#将网格坐标转化为实际的米制坐标
        boundary_groups=boundary_groups,
        # scale=INTERIOR_BOUNDARY_SCALE,
        scale = unit_size
    )
    xz_poly_map = get_xz_poly_map(boundary_groups=boundary_groups, room_ids=room_ids)#为每个房间创建封闭的多边形边界，用于后续物体放置

    ceiling_height = 0
    return HouseStructure(#返回值，主要是和墙壁边界处理有关的
